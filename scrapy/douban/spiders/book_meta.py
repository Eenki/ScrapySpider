...
class BookMetaSpider(Spider):
    ...
    sql = 'SELECT * FROM subjects WHERE type="book" AND douban_id NOT IN (SELECT douban_id FROM books) ORDER BY douban_id'
    cursor.execute(sql)
...
-sibling::br]'
    ...
    def get_series(self, meta, response):
        regx = '//a[preceding-sibling::span[text()="丛书:"]][following-sibling::br]/text()'
    ...
    def get_publisher(self, meta, response):
        regx = '//text()[preceding-sibling::span[text()="出版社:"]][following-sibling::br]'
        data = response.xpath(regx).extract()
        if data:
    ...
    def get_publish_date(self, meta, response):
        regx = '//text()[preceding-sibling::span[text()="出版年:"]][following-sibling::br]'
        data = response.xpath(regx).extract()
        if data:
    ...
    def get_pages(self, meta, response):
        regx = '//text()[preceding-sibling::span[text()="页数:"]][following-sibling::br]'
        data = response.xpath(regx).extract()
        if data:
            meta['pages'] = data[0]
        return meta
    ...
    def get_price(self, meta, response):
        regx = '//text()[preceding-sibling::span[text()="定价:"]][following-sibling::br]'
        data = response.xpath(regx).extract()
        if data:
            meta['price'] = data[0][:-1]
        return meta
    ...
    def get_binding(self, meta, response):
        regx = '//text()[preceding-sibling::span[text()="装帧:"]][following-sibling::br]'
        data = response.xpath(regx).extract()
        if data:
            meta['binding'] = data[0]
        return meta
    ...
    def get_isbn(self, meta, response):
        regx = '//text()[preceding-sibling::span[text()="ISBN:"]][following-sibling::br]'
        data = response.xpath(regx).extract()
        if data:
            meta['isbn'] = data[0]
        return meta
