...
class MovieMetaSpider(Spider):
    ...
    def get_official_site(self, meta, response):
        regx = '//a[preceding-sibling::span[text()="官方网站:"]][following-sibling::br]/@href'
        data = response.xpath(regx).extract()
        if data:
    ...
    def get_regions(self, meta, response):
        regx = '//text()[preceding-sibling::span[text()="制片国家/地区:"]][following-sibling::br]'
        data = response.xpath(regx).extract()
        if data:
            meta['regions'] = data[0]
        return meta
    ...
    def get_languages(self, meta, response):
        regx = '//text()[preceding-sibling::span[text()="语言:"]][following-sibling::br]'
        data = response.xpath(regx).extract()
        if data:
    ...
    def get_alias(self, meta, response):
        regx = '//text()[preceding-sibling::span[text()="又名:"]][following-sibling::br]'
        data = response.xpath(regx).extract()
        if data:
    ...
    def get_imdb_id(self, meta, response):
        regx = '//a[preceding-sibling::span[text()="IMDb链接:"]][following-sibling::br]/@href'
        data = response.xpath(regx).extract()
        if data:
